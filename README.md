...
| 等锁定再决策 | 16–31 手 | **300（消 2–3 行）× 连续两局** |

> 单局分数噪声很大（同一版本也出现过 0 分），所以这里只报**多局可复现**的结果。
> 离人类水平还远，瓶颈仍在执行层时序，不在决策质量（golden set 已 10/10）。
...
- **俄罗斯方块稳定在 300 分 / 消 2–3 行**，离人类水平还远。决策层已校准到 100%
  （golden set 10/10），瓶颈在**执行层时序**：单步仍需 ~1.3s，方块在这期间会多掉
  1–2 行，落点仍会偏离选中的列。下一步应该按下落速度做提前量补偿，或找到硬降入口。
- 单局分数噪声大（同一版本出现过 0~300 分），评估必须跑多局。
